function[T_air_out, T_peb, T_fluid, dn_out, mass_per_dh, p_air_out, h_vol_lof, Heat_Ex_loss, p_bed, moles_in_slice, Delta_T_fluid, Delta_n] = ...
    Reheat_charge(dn, N_dot, T_air_in, p_air_in, c_air_mol, T_peb, T_fluid, length_PB, r_bed, dl, c_peb, k_peb, ...
    lambda_ins, ins_th, p_store, V_store, T_store, T_am, p_bed, moles_in_slice, Delta_T_fluid, Delta_n, dn_out, voidage)
...
G = (N_dot*0.029)/(A);
% h_vol_lof = 652*(G/pebble_size)^0.7; %lof and hawley
h_vol_lof = 700*(G/pebble_size)^0.76; % J.Pascal Coutier, E.A. Farber (newer)
h_vol = h_vol_lof;
...
if(p_air_in>p_store)
    ...
    % need to define moles in slice, dn_out, Delta_n, Delta_T_fluid outside
    % of this program
    
    dns(1) = dn;
    dns(2) = dns(1) + moles_in_slice(1)*(1-T_fluid(1)/(T_fluid(1)+Delta_T_fluid(1))) - Delta_n(1)*(T_fluid(1)/(T_fluid(1)+Delta_T_fluid(1)));
    for i = 3:n_slices+1
        dns(i) = dns(i-1) + moles_in_slice(i-1)*(1-T_fluid(i-1)/(T_fluid(i-1)+Delta_T_fluid(i-1))) ... 
             - Delta_n(i-1)*(T_fluid(i-1)/(T_fluid(i-1)+Delta_T_fluid(i-1)));
    end
    % if the store pressure is increasing and this is the stage before the
    % store then the pressurer in the bed must increase too.
    p_bed = p_bed + dn_out*8.31*T_store/(100000*V_store);
else
...
% moles_in_slice = ((p_bed*100000*void_vol_slice)./(8.31*T_fluid))-dns(2:n_slices+1);
...
% Delta n is the amount of moles that we expect the slice to gain in the
% next dn addition to match the pressure increase in the store at its
% current temperature
Delta_n = dn_out*(void_vol_slice*T_store./(V_store*T_fluid));
